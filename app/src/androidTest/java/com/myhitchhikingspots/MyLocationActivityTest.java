...
public class MyLocationActivityTest extends ActivityInstrumentationTestCase2<TrackLocationBaseActivity> {
    public static final String TAG = "MyLocationActivityTest";

    /**
     * The name of the mock location.
     */
    public static final String NORTH_POLE =
            "com.myhitchhikingspots" + ".NORTH_POLE";
     ...
     */
    private TrackLocationBaseActivity mTrackLocationBaseActivity;

    public MyLocationActivityTest() {
        super(TrackLocationBaseActivity.class);
    }
    ...
    protected void setUp() throws Exception {
        super.setUp();
        mTrackLocationBaseActivity = getActivity();
    ...
    public void testUsingMockLocation() {
        ...
        assertEquals(testLocation.getLatitude(), mTrackLocationBaseActivity.mCurrentLocation.getLatitude(),
                0.000001f);
        assertEquals(testLocation.getLongitude(), mTrackLocationBaseActivity.mCurrentLocation.getLongitude(),
                 0.000001f);
        //assertEquals(String.valueOf(testLocation.getLatitude()),
        //        mTrackLocationBaseActivity.mLatitudeTextView.getText().toString());
        //assertEquals(String.valueOf(testLocation.getLongitude()),
        //        mTrackLocationBaseActivity.mLongitudeTextView.getText().toString());
    }
    ...
    private void ensureGoogleApiClientConnection() {
        if (!mTrackLocationBaseActivity.mGoogleApiClient.isConnected()) {
            mTrackLocationBaseActivity.mGoogleApiClient.blockingConnect();
        }
    }
    ...
    private void confirmPreconditions() {
        assertNotNull("mTrackLocationBaseActivity is null", mTrackLocationBaseActivity);
        assertTrue("GoogleApiClient is not connected", mTrackLocationBaseActivity.mGoogleApiClient.isConnected());
    }
    ...
    private void clickStartUpdatesButton() {
        mTrackLocationBaseActivity.runOnUiThread(new Runnable() {
            public void run() {
               // mTrackLocationBaseActivity.mSaveSpotButton.performClick();
            }
        });
    }
    ...
    private void setMockLocation(final Location mockLocation) {
        ...
        LocationServices.FusedLocationApi.setMockMode(mTrackLocationBaseActivity.mGoogleApiClient, true)
                    ...
                    public void onResult(Status status) {
                        if (status.isSuccess()) {
                            ...
                            LocationServices.FusedLocationApi.setMockLocation(
                                    mTrackLocationBaseActivity.mGoogleApiClient,
                                    mockLocation
                            ).setResultCallback(new ResultCallback<Status>() {
                                @Override
                                public void onResult(Status status) {
